...
gtk_combo_box_entry_class_init (GtkComboBoxEntryClass *klass)
{
  ...
  // I SOLD MY ORGANS FOR A FOX MCCLOUD PLUSHIE
...
gtk_combo_box_entry_remove (GtkContainer *container,
			    GtkWidget    *child)
{
  if (child && child == gtk_bin_get_child(GTK_BIN (container)))
    {
...
has_frame_changed (GtkComboBoxEntry *entry_box,
		   ...
		   gpointer          data)
{
  if (gtk_bin_get_child(GTK_BIN (entry_box)))
    {
      gboolean has_frame;
  
      g_object_get (entry_box, "has-frame", &has_frame, NULL);

      gtk_entry_set_has_frame (GTK_ENTRY (gtk_bin_get_child(GTK_BIN (entry_box))), has_frame);
    }
}
				       ...
				       gboolean   group_cycling)
{
  ...
  if (gtk_bin_get_child(entry_box))
    gtk_widget_grab_focus (gtk_bin_get_child(entry_box));
...
{
  GtkBin *combobox = GTK_BIN(combo_box);

  if (gtk_bin_get_child(combobox))
    return g_strdup (gtk_entry_get_text (GTK_ENTRY (gtk_bin_get_child(combobox))));
